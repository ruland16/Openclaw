// Mission Control Dashboard - Fixed WebSocket Version
class FixedDashboard {
    ...
    showConnectionHelp() {
        ...
        if (statusElement) {
            statusElement.innerHTML = '<i class="fas fa-question-circle"></i><span>Click to retry</span>';
            statusElement.style.cursor = 'pointer';
            statusElement.title = 'Click to retry WebSocket connection';
        }
    }
    ...
    async loadInitialData() {
        try {
            ...
            // Load agents
            try {
                const agentsResponse = await fetch(`${this.apiBaseUrl}/api/agents`);
            ...
            // Load tasks
            try {
                const tasksResponse = await fetch(`${this.apiBaseUrl}/api/tasks`);
            ...
            // Load activities
            try {
                const activitiesResponse = await fetch(`${this.apiBaseUrl}/api/memory/activities`);
            ...
            // Load metrics
            try {
                const metricsResponse = await fetch(`${this.apiBaseUrl}/api/metrics`);
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    window.dashboard = new FixedDashboard();
});
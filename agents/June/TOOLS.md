# TOOLS.md - June's Personal Configuration

## Email Configuration

**Primary Tool:** `gog` CLI (Google Workspace)
- **Account:** ruland16@gmail.com
- **Auth:** File-backed keyring with `GOG_KEYRING_BACKEND=file` and `GOG_KEYRING_PASSWORD` env vars
- **Access:** June has read/write access to Gmail for cleanup tasks

**Email Cleanup Script:**
- **Location:** `/home/user/.openclaw/workspace/agents/Lui/scripts/clean-email.sh`
- **Usage:** `bash /home/user/.openclaw/workspace/agents/Lui/scripts/clean-email.sh [--dry-run]`
- **What it does:** Trashes emails from known junk senders (Vista Auction, Amazon, AliExpress, USPS, Google, Kia Connect, NC Virtual Academy)
- **Configured for:** ruland16@gmail.com

**Manual gog commands:**
```bash
export GOG_KEYRING_BACKEND=file
export GOG_KEYRING_PASSWORD='SecureKeyring123!'
export GOG_ACCOUNT=ruland16@gmail.com

# Search inbox
gog gmail search 'in:inbox' --max 20 --json

# Trash specific thread
gog gmail trash <threadId> --no-input

# Archive thread
gog gmail archive <threadId> --no-input
```

## Family Information
*Note: Fill in specific details as you learn them*

### Family Members
1. **Russ** (User)
   - Primary contact
   - Preferences: 
   - Health notes:
   - Communication style:

2. **Spouse**
   - Name: 
   - Preferences:
   - Health notes:
   - Communication style:

3. **Child 1**
   - Name: 
   - Age:
   - School/Activities:
   - Health notes:
   - Preferences:

4. **Child 2**
   - Name:
   - Age:
   - School/Activities:
   - Health notes:
   - Preferences:

5. **Child 3**
   - Name:
   - Age:
   - School/Activities:
   - Health notes:
   - Preferences:

6. **Child 4**
   - Name:
   - Age:
   - School/Activities:
   - Health notes:
   - Preferences:

7. **Child 5**
   - Name:
   - Age:
   - School/Activities:
   - Health notes:
   - Preferences:

## Home Details
*North Carolina Family Home*

### Property Information
- **Address:** [Redacted for privacy]
- **Property Type:** Single family home
- **Year Built:** 
- **Square Footage:** 
- **Lot Size:** 

### Critical Systems
1. **HVAC System**
   - Type:
   - Last service:
   - Filter size:
   - Filter change frequency: Monthly
   - Service company:

2. **Water Heater**
   - Type:
   - Age:
   - Capacity:
   - Maintenance schedule:

3. **Appliances** (list major ones)
   - Refrigerator:
   - Oven/Stove:
   - Dishwasher:
   - Washing Machine:
   - Dryer:

4. **Lawn Care**
   - Grass type: Bermuda grass
   - Mowing schedule: Weekly during growing season
   - Fertilization schedule: 
   - Aeration schedule:
   - Weed control:

### Home Maintenance Schedule
*Based on North Carolina climate*

#### Monthly
- Change HVAC filters
- Test smoke/CO detectors
- Clean garbage disposal
- Inspect fire extinguishers

#### Quarterly
- Clean gutters
- Inspect roof
- Test garage door auto-reverse
- Service HVAC system

#### Semi-Annual
- Deep clean carpets
- Pressure wash exterior
- Inspect siding/paint
- Test sump pump (if applicable)

#### Annual
- Clean dryer vent
- Inspect attic insulation
- Service water heater
- Seal driveway/walkways
- Winterize irrigation system

## Health & Wellness Tracking

### Medical Providers
- **Primary Care:** 
- **Dentist:** 
- **Pediatrician:** 
- **Specialists:** 

### Medication Schedule
*Track for each family member*

### Allergy Information
- **Seasonal:** Pollen (spring), Ragweed (fall)
- **Food:** 
- **Environmental:** 

### Immunization Records
*Keep track of due dates*

## Calendar Configuration

### Color Coding System
- **Red:** Medical appointments
- **Blue:** School events
- **Green:** Family activities
- **Yellow:** Work commitments
- **Purple:** Personal development
- **Orange:** Home maintenance

### Notification Preferences
- **Medical:** 1 day + 2 hours before
- **School:** 1 day before
- **Family:** 3 hours before
- **Maintenance:** 1 week + 1 day before

## Communication Preferences

### Preferred Channels
- **Urgent:** Text/Phone call
- **Routine:** Telegram/Email
- **Planning:** Shared calendar
- **Documentation:** Notes app

### Response Time Expectations
- **Emergency:** Immediate
- **Urgent:** Within 1 hour
- **Important:** Within 4 hours
- **Routine:** Within 24 hours

## Personal Development Tracking

### Study Areas
1. **Christian Theology**
   - Current focus:
   - Resources:
   - Study schedule:
   - Key insights:

2. **Reading List**
   - Current book:
   - Next up:
   - Reading pace:
   - Discussion points:

3. **Skill Development**
   - Current focus:
   - Practice schedule:
   - Progress metrics:
   - Resources needed:

## Meal Planning System

### Dietary Preferences
- **Restrictions:** 
- **Preferences:** 
- **Allergies:** 
- **Favorite meals:** 

### Grocery Template
```
Weekly Grocery List - [Date]

Produce:
- 
- 

Proteins:
- 
- 

Dairy:
- 
- 

Pantry:
- 
- 

Household:
- 
- 
```

## Emergency Contacts

### Family
- Spouse: [Phone]
- Emergency contact 1: [Name, Phone, Relationship]
- Emergency contact 2: [Name, Phone, Relationship]

### Services
- Poison Control: 1-800-222-1222
- Electric Company: 
- Water Company: 
- Gas Company: 
- HVAC Emergency: 
- Plumber: 
- Electrician: 

### Medical
- Primary Care: 
- Urgent Care: 
- Hospital: 
- Pharmacy: 

## Template Files

### Morning Brief Template
```
Family Brief - [Date] [Day of Week]

Good morning! Here's what's on tap for today:

🌤️ Weather: [High/Low, conditions]

📅 Today's Schedule:
1. [Time] - [Event]
2. [Time] - [Event]
3. [Time] - [Event]

💊 Health Reminders:
- [Medication/Appointment]

🎉 Special Notes:
- [Birthdays/Anniversaries/Special events]

🏡 Home Tasks:
- [Quick maintenance item]

📚 Personal Focus:
- [Study/reading goal]

Have a great day! 🌸
```

### Evening Check-in Template
```
Evening Check-in - [Date]

Today's accomplishments:
✅ [Completed task 1]
✅ [Completed task 2]
✅ [Completed task 3]

Tomorrow's preview:
📅 [Key event 1]
📅 [Key event 2]

Wellness check:
💤 Sleep goal: [hours]
🥗 Nutrition: [note]
🧘 Mindfulness: [note]

Family highlight:
🌟 [Positive moment from today]

Rest well! 🌙
```

## Notes
- Update this file as you learn more about the family
- Keep sensitive information secure
- Review and update quarterly
- Share relevant updates with Lui for organizational knowledge